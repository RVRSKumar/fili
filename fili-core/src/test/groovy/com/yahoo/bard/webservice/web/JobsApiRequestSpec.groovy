...
import com.yahoo.bard.webservice.jobs.ApiJobStoreFilter
import com.yahoo.bard.webservice.jobs.ApiJobStoreFilterOperation
...
import com.yahoo.bard.webservice.jobs.JobTestUtils
...
class JobsApiRequestSpec extends Specification {
    ...
    def "buildJobStoreFilter correctly parses a single filter query and returns ApiJobStoreFilter"() {
        ...
        when:
        Set<ApiJobStoreFilter> filters = defaultJobsApiRequest.buildJobStoreFilter(filterQuery)

        then:
        filters.size() == 1
        ApiJobStoreFilter filter = filters[0]
        filter.jobField?.name == "userId"
        filter.operation == ApiJobStoreFilterOperation.valueOf("eq")
    ...
    def "buildJobStoreFilter correctly parses multiple filter query"() {
        ...
        ApiJobStoreFilter filter1 = filters[0]
        filter1.jobField?.name == "userId"
        filter1.operation == ApiJobStoreFilterOperation.valueOf("eq")
        filter1.values == ["foo"] as Set

        ApiJobStoreFilter filter2 = filters[1]
        filter2.jobField?.name == "status"
        filter2.operation == ApiJobStoreFilterOperation.valueOf("eq")
    ...
    def "getFilteredJobViews returns JobRows that satisfy the given filter"() {
        setup:
        ApiJobStoreFilter filter = new ApiJobStoreFilter(USER_ID, ApiJobStoreFilterOperation.eq, ["Number 1", "Number 2"] as Set)
